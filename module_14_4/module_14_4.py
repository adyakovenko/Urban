...
products.append(Product('yellow', 90, 'желтая таблетка'))

crud_functions.clear_db()
crud_functions.initiate_db()
for product in products:
    crud_functions.add_product(product.name, product.comment, product.price)
texts = crud_functions.get_all_products()
...
async def get_buying_list(message):
    for i in range(len(products)):
        with open(f'files/{products[i].picture_name}', 'rb') as img:
            await message.answer_photo(img, texts[i])
